...
import { Users, Search, Plus, Edit2, Trash2, Award } from 'lucide-react';
...
export default function AdminCustomers() {
    ...
    const handleEdit = (c) => {
        setFormData({ name: c.name, phone: c.phone, email: c.email || '', loyalty_points: c.loyalty_points || 0 });
        setEditId(c.id);
        setShowForm(true);
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            toast.error("Cannot delete customer. They might be linked to existing sales.");
        }
    };
    ...
    return (
        <div style={{ animation: 'fadeIn 0.5s' }}>
            <div className="flex-between mb-4">
                <h2>Customer CRM & Loyalty</h2>
                <button className="btn btn-primary" onClick={() => { setShowForm(!showForm); setEditId(null); setFormData({name:'', phone:'', email:'', loyalty_points:0}); }}>
            ...
            {showForm && (
                <div className="glass-panel" style={{ padding: '2rem', marginBottom: '2rem' }}>
                    <h3 className="mb-4">{editId ? 'Edit Customer' : 'Add New Customer'}</h3>
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        ...
                        <div className="form-group">
                            <label className="form-label">Loyalty Points (Manual Override)</label>
                            <input type="number" className="form-input" value={formData.loyalty_points} onChange={e => setFormData({...formData, loyalty_points: parseInt(e.target.value) || 0})} />
                        </div>
            ...
            <div className="glass-panel" style={{ padding: '2rem', overflowX: 'auto' }}>
                {loading ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan="6" className="text-center text-muted" style={{ padding: '2rem' }}>No customers found</td></tr>
                            ) : filtered.map(c => (
                                <tr key={c.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <td style={{ padding: '1rem', display: 'flex', gap: '10px' }}>
                                        <button onClick={() => handleEdit(c)} className="btn" style={{ padding: '6px 12px', background: 'rgba(99, 102, 241, 0.2)', color: 'var(--primary-color)', border: 'none' }}><Edit2 size={16} /></button>
                                        <button onClick={() => handleDelete(c.id)} className="btn" style={{ padding: '6px 12px', background: 'rgba(239, 68, 68, 0.2)', color: 'var(--danger)', border: 'none' }}><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
